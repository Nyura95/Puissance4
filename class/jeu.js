...
Jeu.prototype.jouer = function(colonne){
  ...
  if (ligne === undefined) {
    document.getElementById("msgInfo").innerHTML = "Il n'y a plus de place dans la colonne.";
    return;
  }
...
Jeu.prototype.gagner = function(ligne, colonne, classNom) {
  ...
  for (var i = Math.max(decalage, 0); i < Math.min(this.lig, this.col + decalage); i++) {
  ...
  for (var i = Math.max(decalage - this.col + 1, 0); i < Math.min(this.lig, decalage + 1); i++) {
